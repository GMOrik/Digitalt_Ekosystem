...
public class Konsument extends Liv{
    ...
    private synchronized void ändraVinkel(){
        ...
        if (!växtlista.isEmpty() && platsINäringskedjan == 0 && hunger <= (int)(standardhunger*0.3 + 0.5)){ //se över hungervillkor för leta mat/para sig
            hoppaMotNärmstaIndivid(växtlista, false);
        }

        
        //4. En individ med högre plats i näringskedjan syns
        //borde dock kanske finnas kontroll mot hörn (över om sidorna ofta är försumbara vid större simuleringar)
        if (!hotlista.isEmpty()){
